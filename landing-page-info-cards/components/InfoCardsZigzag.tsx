...
const Card: React.FC<CardProps> = ({ title, text, index, isLeft }) => (
  <div className={`flex items-center ${isLeft ? "flex-row" : "flex-row-reverse"} gap-8 group`}>
    {/* Number indicator */}
    <div className="flex-shrink-0 w-16 h-16 rounded-full bg-[#142152] text-white flex items-center justify-center text-xl font-bold group-hover:bg-[#FFD539] group-hover:text-[#142152] transition-colors">
      {`0${index + 1}`}
    </div>

    {/* Content */}
    <div
      className={`flex-1 bg-white p-6 rounded-lg shadow-lg transform transition-all duration-300 ${
...
export default function InfoCardsZigzag() {
  const cards = [
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-4xl mx-auto space-y-12">
          {cards.map((card, index) => (
            <Card key={index} {...card} index={index} isLeft={index % 2 === 0} />
          ))}
        </div>
      </div>
    </section>
